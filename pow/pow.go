...
func (pow *PowService) CollectTransactions(MsgBlock *types.Block) int {
	txs := 0
	transactionsPool := pow.localNode.GetTxsInPool()
...
func (pow *PowService) CreateCoinBaseTx(nextBlockHeight uint32, addr string) (*types.Transaction, error) {
...
func (pow *PowService) GenerateBlock(addr string) (*types.Block, error) {
	...
	for _, tx := range txsByFeeDesc {
		...
		fee := GetTxFee(tx, DefaultChain.AssetID)
		if fee != tx.Fee {
			continue
		}
		msgBlock.Transactions = append(msgBlock.Transactions, tx)
...
func (pow *PowService) SolveBlock(MsgBlock *types.Block, ticker *time.Ticker) bool {
	genesisHash, err := DefaultChain.Store.GetBlockHash(0)
	if err != nil {
		return false
	}
...
func (pow *PowService) BroadcastBlock(MsgBlock *types.Block) error {
...
func NewPowService(localNode protocol.Noder) *PowService {
	pow := &PowService{
		...
		started:      false,
		manualMining: false,
		MsgBlock:     msgBlock{BlockData: make(map[string]*types.Block)},
		localNode:    localNode,
	}
