...
func CheckTransactionSanity(txn *types.Transaction) ErrCode {
...
func CheckTransactionContext(txn *types.Transaction) ErrCode {
	// check if duplicated with transaction in ledger
	if exist := DefaultChain.Store.IsTxHashDuplicate(txn.Hash()); exist {
	...
	for _, input := range txn.Inputs {
		...
		referTxn, _, err := DefaultChain.Store.GetTransaction(referHash)
		if err != nil {
		...
		if referTxn.IsCoinBaseTx() {
			...
			currentHeight := DefaultChain.Store.GetHeight()
...
func CheckTransactionInput(txn *types.Transaction) error {
	if txn.IsCoinBaseTx() {
		if len(txn.Inputs) != 1 {
...
func CheckTransactionOutput(txn *types.Transaction) error {
	if txn.IsCoinBaseTx() {
...
func CheckTransactionUTXOLock(txn *types.Transaction) error {
	if txn.IsCoinBaseTx() {
		return nil
	}
	if len(txn.Inputs) <= 0 {
	...
	references, err := DefaultChain.Store.GetTxReference(txn)
	if err != nil {
...
func CheckTransactionSize(txn *types.Transaction) error {
	size := txn.GetSize()
...
func CheckAssetPrecision(txn *types.Transaction) error {
	if len(txn.Outputs) == 0 {
		return nil
	}
	assetOutputs := make(map[Uint256][]*types.Output, len(txn.Outputs))
...
func CheckTransactionSignature(txn *types.Transaction) error {
...
func CheckTransactionPayload(txn *types.Transaction) error {
	...
	case *types.PayloadRegisterIdentification:
	default:
...
func CheckRechargeToSideChainTransaction(txn *types.Transaction) error {
	...
	if exist := DefaultChain.Store.IsMainchainTxHashDuplicate(mainchainTxhash); exist {
	...
	genesisHash, _ := DefaultChain.Store.GetBlockHash(uint32(0))
...
func CheckTransferCrossChainAssetTransaction(txn *types.Transaction) error {
