...
func Init(store IChainStore) error {
	...
	height, err := DefaultChain.Store.InitWithGenesisBlock(genesisBlock)
	if err != nil {
		return errors.New("[Blockchain], InitLevelDBStoreWithGenesisBlock failed, " + err.Error())
	}

	DefaultChain.UpdateBestHeight(height)
	return nil
}
...
func (bc *Blockchain) IsDoubleSpend(Tx *types.Transaction) bool {
	return bc.Store.IsDoubleSpend(Tx)
}

//Get the Asset from store.
func (bc *Blockchain) GetAsset(assetId Uint256) (*types.Asset, error) {
	asset, err := bc.Store.GetAsset(assetId)
	if err != nil {
...
func (bc *Blockchain) BlockInLedger(hash Uint256) bool {
	return bc.Store.IsBlockInStore(hash)
}
...
func (bc *Blockchain) AddBlock(block *types.Block) (bool, bool, error) {
	bc.mutex.Lock()
...
func (bc *Blockchain) GetHeader(hash Uint256) (*types.Header, error) {
	header, err := bc.Store.GetHeader(hash)
	if err != nil {
...
func (bc *Blockchain) ContainsTransaction(hash Uint256) bool {
	...
	_, _, err := bc.Store.GetTransaction(hash)
	if err != nil {
		return false
	}
	return true
...
func (bc *Blockchain) CurrentBlockHash() Uint256 {
	return bc.Store.GetCurrentBlockHash()
}
...
func (bc *Blockchain) AddOrphanBlock(block *types.Block) {
...
func (bc *Blockchain) LoadBlockNode(blockHeader *types.Header, hash *Uint256) (*BlockNode, error) {
...
func (bc *Blockchain) GetPrevNodeFromBlock(block *types.Block) (*BlockNode, error) {
	// Genesis block.
	prevHash := block.Header.Previous
...
func (bc *Blockchain) ReorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	for e := detachNodes.Front(); e != nil; e = e.Next() {
		n := e.Value.(*BlockNode)
		block, err := bc.Store.GetBlock(*n.Hash)
		if err != nil {
			return err
		}
...
//// the main (best) chain.
func (bc *Blockchain) DisconnectBlock(node *BlockNode, block *types.Block) error {
	...
	err = DefaultChain.Store.RollbackBlock(*node.Hash)
	if err != nil {
		return err
	}
...
// (best) chain.
func (bc *Blockchain) ConnectBlock(node *BlockNode, block *types.Block) error {
...
func (bc *Blockchain) BlockExists(hash *Uint256) (bool, error) {
	...
	return bc.BlockInLedger(*hash), nil
}

func (bc *Blockchain) maybeAcceptBlock(block *types.Block) (bool, error) {
	...
	err = PowCheckBlockContext(block, prevNode)
	if err != nil {
...
func (bc *Blockchain) ConnectBestChain(node *BlockNode, block *types.Block) (bool, error) {
...
//2. isOphan
//3. error
func (bc *Blockchain) ProcessBlock(block *types.Block, timeSource MedianTimeSource, flags uint32) (bool, bool, error) {
	blockHash := block.Hash()
...
func (bc *Blockchain) LatestBlockLocator() ([]*Uint256, error) {
	bc.mutex.RLock()
	defer bc.mutex.RUnlock()
	if bc.BestChain == nil {
		...
		// Get Current Block
		blockHash := bc.Store.GetCurrentBlockHash()
		return bc.blockLocatorFromHash(&blockHash), nil
	}

	// The best chain is set, so use its hash.
	return bc.blockLocatorFromHash(bc.BestChain.Hash), nil
}
func (bc *Blockchain) AddNodeToIndex(node *BlockNode) {
	bc.IndexLock.Lock()
	defer bc.IndexLock.Unlock()

	bc.Index[*node.Hash] = node
}
func (bc *Blockchain) RemoveNodeFromIndex(node *BlockNode) {
	bc.IndexLock.Lock()
	defer bc.IndexLock.Unlock()
	delete(bc.Index, *node.Hash)
}
func (bc *Blockchain) LookupNodeInIndex(hash *Uint256) (*BlockNode, bool) {
	bc.IndexLock.Lock()
	defer bc.IndexLock.Unlock()
	node, exist := bc.Index[*hash]

	return node, exist
}

func (bc *Blockchain) BlockLocatorFromHash(inhash *Uint256) []*Uint256 {
	bc.mutex.RLock()
	defer bc.mutex.RUnlock()
	return bc.blockLocatorFromHash(inhash)
}
func (bc *Blockchain) blockLocatorFromHash(inhash *Uint256) []*Uint256 {
	...
	if hash.IsEqual(bc.GenesisHash) {
		return locator
	}
	...
	for len(locator) < MaxBlockLocatorsPerMsg-1 {
		...
		h, err := bc.Store.GetBlockHash(uint32(blockHeight))
		if err != nil {
	...
	locator = append(locator, &bc.GenesisHash)

	return locator
}

func (bc *Blockchain) LatestLocatorHash(locator []*Uint256) *Uint256 {
...
func (bc *Blockchain) MedianAdjustedTime() time.Time {
	newTimestamp := bc.TimeSource.AdjustedTime()
	minTimestamp := bc.MedianTimePast.Add(time.Second)
