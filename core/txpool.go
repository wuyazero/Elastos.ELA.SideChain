...
func (pool *TxPool) Init() {
	...
	defer pool.Unlock()
	pool.txnCnt = 0
	pool.inputUTXOList = make(map[string]*types.Transaction)
	//pool.issueSummary = make(map[Uint256]Fixed64)
	pool.txnList = make(map[Uint256]*types.Transaction)
	pool.mainchainTxList = make(map[Uint256]*types.Transaction)
}

//append transaction to txnpool when check ok.
//1.check  2.check with ledger(db) 3.check with pool
func (pool *TxPool) AppendToTxnPool(txn *types.Transaction) ErrCode {
	...
	txn.Fee = GetTxFee(txn, DefaultChain.AssetID)
...
func (pool *TxPool) GetTxInPool(txId Uint256) (*types.Transaction, bool) {
	pool.RLock()
	defer pool.RUnlock()
...
func (pool *TxPool) GetTxsInPool() map[Uint256]*types.Transaction {
	pool.RLock()
	defer pool.RUnlock()
...
func (pool *TxPool) CleanSubmittedTransactions(block *types.Block) error {
...
func (pool *TxPool) GetTransaction(hash Uint256) *types.Transaction {
	pool.RLock()
	defer pool.RUnlock()
...
func (pool *TxPool) verifyTransactionWithTxnPool(txn *types.Transaction) ErrCode {
...
func (pool *TxPool) removeTransaction(txn *types.Transaction) {
	//1.remove from txnList
	pool.delFromTxList(txn.Hash())
	//2.remove from UTXO list map
	result, err := DefaultChain.Store.GetTxReference(txn)
	if err != nil {
...
func (pool *TxPool) verifyDoubleSpend(txn *types.Transaction) error {
	reference, err := DefaultChain.Store.GetTxReference(txn)
	if err != nil {
		return err
	}
	inputs := []*types.Input{}
...
func (pool *TxPool) verifyDuplicateMainchainTx(txn *types.Transaction) error {
...
func (pool *TxPool) cleanUTXOList(txs []*types.Transaction) {
	for _, txn := range txs {
		inputUtxos, _ := DefaultChain.Store.GetTxReference(txn)
...
func (pool *TxPool) cleanTransactionList(txns []*types.Transaction) error {
	cleaned := 0
	txnsNum := len(txns)
...
func (pool *TxPool) cleanMainchainTx(txs []*types.Transaction) {
	for _, txn := range txs {
		if txn.IsRechargeToSideChainTx() {
...
func (pool *TxPool) addToTxList(txn *types.Transaction) bool {
	pool.Lock()
	defer pool.Unlock()
	txnHash := txn.Hash()
...
func (pool *TxPool) copyTxList() map[Uint256]*types.Transaction {
	pool.RLock()
	defer pool.RUnlock()
	txnMap := make(map[Uint256]*types.Transaction, len(pool.txnList))
...
func (pool *TxPool) getInputUTXOList(input *types.Input) *types.Transaction {
	pool.RLock()
	defer pool.RUnlock()
	return pool.inputUTXOList[input.ReferKey()]
}

func (pool *TxPool) addInputUTXOList(tx *types.Transaction, input *types.Input) bool {
	pool.Lock()
	defer pool.Unlock()
	id := input.ReferKey()
...
func (pool *TxPool) delInputUTXOList(input *types.Input) bool {
	pool.Lock()
	defer pool.Unlock()
	id := input.ReferKey()
	_, ok := pool.inputUTXOList[id]
...
func (pool *TxPool) addMainchainTx(txn *types.Transaction) {
	pool.Lock()
	defer pool.Unlock()
	rechargePayload := txn.Payload.(*types.PayloadRechargeToSideChain)
	hash, err := rechargePayload.GetMainchainTxHash()
...
func (pool *TxPool) MaybeAcceptTransaction(txn *types.Transaction) error {
	txHash := txn.Hash()
...
func GetTxFee(tx *types.Transaction, assetId Uint256) Fixed64 {
...
func GetTxFeeMap(tx *types.Transaction) (map[Uint256]Fixed64, error) {
	...
	reference, err := DefaultChain.Store.GetTxReference(tx)
	if err != nil {
		return nil, err
	}
