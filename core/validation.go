...
func RunPrograms(tx *types.Transaction, hashes []Uint168, programs []*types.Program) error {
	if tx == nil {
...
func GetTxProgramHashes(tx *types.Transaction) ([]Uint168, error) {
	if tx == nil {
	...
	references, err := DefaultChain.Store.GetTxReference(tx)
	if err != nil {
